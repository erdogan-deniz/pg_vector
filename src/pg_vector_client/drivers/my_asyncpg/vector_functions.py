...
from .data import VectorSimilarityOperators
from asyncpg import (
    InterfaceError,
    SQLRoutineError,
    InvalidNameError,
...
async def add_vector(
    ...
    try:
        sql_query: str = f"""
                          INSERT INTO {table_name} ({vec_col_name})
                          VALUES ($1);
                          """
...
async def get_similar_vectors(
    ...
    res_count: int = 5,
    vec_sim_oper_tyoe: VectorSimilarityOperators =
    ...
    try:
        sql_query: str = f"""
                          SELECT *
                          FROM {table_name}
                          ORDER BY {vec_col_name} {vec_sim_oper_tyoe.value} $1
                          LIMIT {res_count};
                          """

        vecs: Any = await conn.fetch(
            sql_query,
            vec
        )
    except InterfaceError as interf_err:
        await logger.aerror(
            f"\nInterfaceError: {str(interf_err).capitalize()}.\n"
            f"File name is: {__file__}.\n"
            f"Function name is: {get_similar_vectors.__name__}.\n"
        )
    except SQLRoutineError as sql_rout_err:
        await logger.aerror(
            f"\nSQLRoutineError: {str(sql_rout_err).capitalize()}.\n"
